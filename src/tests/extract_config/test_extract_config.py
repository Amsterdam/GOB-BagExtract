...
class TestExtractConfig:
    ...
    def test_get_extract_definition(self, tests_dir):
        ...
        gemeente = '0457'
        def1 = get_extract_definition('bag_test', 'ligplaatsen_test', gemeente)
        def2 = json.loads(Path(tests_dir, "fixtures", "bag_data", "bag.test.json").read_text())
        assert def1 == def2
        def1 = get_extract_definition('bag_test', 'ligplaatsen_test')
        assert def1 == def2

    @pytest.mark.parametrize(
        "catalogue, collection", [
            ("fake_catalogue", "ligplaatsen_test"),
            ("bag_test", "fake_collection"),
    ...
    def test_get_extract_definition_uses_cache(self):
        ...
        get_extract_definition('bag_test', 'ligplaatsen_test', gemeente)
